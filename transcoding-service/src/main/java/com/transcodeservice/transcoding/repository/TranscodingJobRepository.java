...
public interface TranscodingJobRepository extends JpaRepository<TranscodingJob, String> {
    ...
    Page<TranscodingJob> findByUserIdAndStatus(String userId, TranscodingJob.TranscodingStatus status, Pageable pageable);
    
    @Query("SELECT j FROM TranscodingJob j WHERE j.status = 'QUEUED' ORDER BY " +
           "CASE j.priority " +
           "WHEN 'URGENT' THEN 1 " +
           "WHEN 'HIGH' THEN 2 " +
           "WHEN 'NORMAL' THEN 3 " +
